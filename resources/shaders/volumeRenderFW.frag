...
{ 
	...
	for (int i = 0; i < MAX_SAMPLES; ++i)
	{
		...
		float smpl = texture(volumeTex, dataPos).r;	
		...
		//is subtracted from the smpl value to get the alpha from the previous steps.
		...
		float prev_alpha = smpl - (smpl * fragColor.a);
		fragColor.rgb = prev_alpha * vec3(smpl) + fragColor.rgb; 
