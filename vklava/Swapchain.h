...
namespace lava
{
	...
	public:
		Swapchain(const std::shared_ptr<Device>& device, const std::shared_ptr<Surface>& surface, 
			...
			const std::vector<uint32_t>& queueFamilyIndices, vk::SurfaceTransformFlagBitsKHR preTransform, 
		...
		uint32_t acquireNextImage(uint64_t timeout = UINT64_MAX, const std::shared_ptr<Fence>& fence = {});
