...
namespace lava
{
	...
	Fence::Fence(std::shared_ptr<Device> const& device, bool signaled)
	...
	/*void resetFences(vk::ArrayProxy<const std::shared_ptr<Fence>> fences)
	{
		if (!fences.empty())
		{
			...
			for (std::shared_ptr<Fence> const& fence : fences)
			{
	...
	void Queue::submit(vk::ArrayProxy<const SubmitInfo> submitInfos, std::shared_ptr<Fence> const & fenceIn)
	{
	...
	void Queue::submit(std::shared_ptr<CommandBuffer> const & commandBuffer, std::shared_ptr<Fence> const & fence)
	{
