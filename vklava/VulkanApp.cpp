...
namespace lava
{
	VulkanApp::VulkanApp(const char* title, uint32_t width, uint32_t height)
	{
		...
		instance->createDebugReportCallback(vk::DebugReportCallbackCreateInfoEXT(debugFlags, &lava::debugMsgCallback));
#endif
		...
		std::vector<vk::SurfaceFormatKHR> surfaceFormats = _physicalDevice->getSurfaceFormats(_surface);
		...
		std::vector<uint32_t> queueFamilyIndices = getGraphicsPresentQueueFamilyIndices(_physicalDevice, _surface);
		...
		_device = _physicalDevice->createDevice(queueCreateInfos, enabledLayerNames, enabledExtensionNames);
	...
	void VulkanApp::resize(uint32_t width, uint32_t height)
	{
		...
		_framebufferSwapchain.reset();    // need to be reset, before creating a new one!!
		_framebufferSwapchain.reset(new FramebufferSwapchain(_device, _surface, 
			_colorFormat, _depthFormat, _renderPass));
		assert(_framebufferSwapchain->getExtent() == vk::Extent2D(width, height));
	...
	void VulkanApp::paint()
	{
		...
		_framebufferSwapchain->acquireNextFrame();
		doPaint();
		_framebufferSwapchain->present(_graphicsQueue, _renderComplete);
	}
	...
	void VulkanApp::doPaint()
	{
		...
		commandBuffer->begin();
		commandBuffer->beginRenderPass(_renderPass, _framebufferSwapchain->getFramebuffer(), 
			vk::Rect2D({ 0, 0 }, _framebufferSwapchain->getExtent()), { vk::ClearValue(ccv), 
			vk::ClearValue(vk::ClearDepthStencilValue(1.0f, 0)) }, 
			vk::SubpassContents::eInline);
		commandBuffer->setViewport(0, { vk::Viewport(0.0f, 0.0f,
			(float)_framebufferSwapchain->getExtent().width,
			(float)_framebufferSwapchain->getExtent().height, 0.0f, 1.0f) });
		commandBuffer->setScissor(0, { vk::Rect2D({ 0, 0 }, _framebufferSwapchain->getExtent()) });
		commandBuffer->endRenderPass();
		commandBuffer->end();

		_graphicsQueue->submit(SubmitInfo{ { _framebufferSwapchain->getPresentSemaphore() },{ vk::PipelineStageFlagBits::eColorAttachmentOutput }, commandBuffer, _renderComplete });
	}

	void VulkanApp::paintCallback(GLFWwindow * window)
	{
		reinterpret_cast<VulkanApp*>(glfwGetWindowUserPointer(window))->paint();
	}

	void VulkanApp::resizeCallback(GLFWwindow *window, int width, int height)
	{
		reinterpret_cast<VulkanApp*>(glfwGetWindowUserPointer(window))->resize(width, height);
	}

	void VulkanApp::cursorPosCallback(GLFWwindow * window, double xPos, double yPos)
	{
		reinterpret_cast<VulkanApp*>(glfwGetWindowUserPointer(window))->cursorPosEvent(xPos, yPos);
	}

	void VulkanApp::keyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
	{
		reinterpret_cast<VulkanApp*>(glfwGetWindowUserPointer(window))->keyEvent(key, scancode, action, mods);
	}

	void VulkanApp::mouseButtonCallback(GLFWwindow * window, int button, int action, int mods)
	{
		reinterpret_cast<VulkanApp*>(glfwGetWindowUserPointer(window))->mouseButtonEvent(button, action, mods);
	}
