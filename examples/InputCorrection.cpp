...
class PostProcessApp : public VulkanApp
{
public:
  struct
  {
  ...
  PostProcessApp( char const* title, uint32_t width, uint32_t height )
    : VulkanApp( title, width, height )
  {
    ...
    fragmentStage = _device->createShaderPipelineShaderStage(
      LAVA_EXAMPLES_SPV_ROUTE + std::string( "inputCorrectionFbo_frag.spv" ), 
      vk::ShaderStageFlagBits::eFragment
    );
...
int main( int argc, char** argv )
{
  try
  {
    VulkanApp* app = new PostProcessApp( "Postprocess", 800, 600 );
