...
public:
    ...
    : VulkanApp( title, width, height )
  {
    ...
    }
    {
      ...
      pipelines.wireframe = _device->createGraphicsPipeline( pipelineCache,
        ...
        { vertexStage, fragmentStage }, vertexInput, assembly, nullptr, 
        viewport, rasterization, multisample, depthStencil, colorBlend, dynamic,
        _pipelineLayout, _renderPass, 0, pipelines.solid, -1 );
    }
    ...
    //recreateSecondaryCommands( width, height );
  }
  void recreateSecondaryCommands( uint32_t width, uint32_t height, vk::CommandBufferInheritanceInfo info )
  {
    secondCmds.clear();
    ...
    std::array<float, 4> ccv = { 0.2f, 0.3f, 0.3f, 1.0f };
    {
      ...
      // TODO: https://github.com/googlesamples/vulkan-basic-samples/blob/master/API-Samples/secondary_command_buffer/secondary_command_buffer.cpp
      // Left top
      {
        std::shared_ptr<lava::CommandBuffer> cmdBuf = 
          commandPool->allocateCommandBuffer( vk::CommandBufferLevel::eSecondary );
        cmdBuf->beginSimple(
          vk::CommandBufferUsageFlagBits::eOneTimeSubmit |
          vk::CommandBufferUsageFlagBits::eRenderPassContinue, &info );

        cmdBuf->setScissor( 0, vk::Rect2D( { 0, 0 }, { width, height } ) );

        viewport.width = width / 2.0f;
        viewport.height = height / 2.0f;
        cmdBuf->setViewport( 0, viewport );
        cmdBuf->bindGraphicsPipeline( pipelines.solid );
        geometry->render( cmdBuf );

        cmdBuf->end( );

        secondCmds.push_back( cmdBuf );
      }
      ...
      // Right top
      {
        ...
        cmdBuf->setViewport( 0, viewport );
        cmdBuf->bindGraphicsPipeline( pipelines.wireframe );
        geometry->render( cmdBuf );
  ...
  std::vector<std::shared_ptr<lava::CommandBuffer> > secondCmds;
  ...
  void doPaint( void ) override
  {
    ...
    info.framebuffer = *_defaultFramebuffer->getFramebuffer( );
    info.renderPass = *_renderPass;

    updateUniformBuffers( );
    recreateSecondaryCommands( width, height, info );
    ...
    commandBuffer->executeCommands( secondCmds );
...
int main( void )
{
  try
  {
    VulkanApp* app = new MyApp( "Secondary Commands", 1200, 600 );
