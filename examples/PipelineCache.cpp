...
class PipelineCacheApp : public VulkanApp
{
public:
  ...
  PipelineCacheApp( char const* title, uint32_t width, uint32_t height )
    : VulkanApp( title, width, height )
  {
    ...
    else
    {
      ...
      auto props = getPhysicalDevice( )->getDeviceProperties( );
...
int main( void )
{
  try
  {
    VulkanApp* app = new PipelineCacheApp( "Pipeline Cache", 1200, 600 );
